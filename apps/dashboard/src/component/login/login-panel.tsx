...
const LoginPanel: React.FC = (): React.ReactElement => {
  ...
  const { shortSHA, vcsLink } = getBuildVersion(environment);

  return (
    <FlexContainer col grow className={cn('min-h-full', 'min-h-screen')}>
      <LoginBgGradient side="left" />
      <LoginBgGradient side="right" />
      <OuterLink to={landingPage} newPage={false} className={cn('hidden', 'sm:block')}>
      ...
      <FlexContainer justify="center" align="center" fullWidth grow>
        <FlexContainer
          ...
          <Button asChild className="w-full">
            <OuterLink to={'/api/oauth/discord/login'} newPage={false}>
              <FaDiscord />
              {t('loginViaDiscord')}
            </OuterLink>
          </Button>
